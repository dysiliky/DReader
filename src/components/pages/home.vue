<template>
	<div id="wrap" :class="model">
		...
		<div class="amatchtitle" v-show="backHotAndState" :class="model">
			<ul>
				<li v-for="item in matchdata" v-if="item.detailpagecontent.slice(0, 3) != '[非]'" @click="todetailarticleView(item.detailpageid)" :class="model">
					<div class="conHeader">
					...
					<div class="detailCon" v-text="item.detailpagecontent">
					</div>
					<div class="conFooter">
		...
		<!--如果没有匹配的搜索内容，显示为None-->
		<div v-if="showNone" class="showNone">暂无相关内容</div>
		<!--回到顶部-->
