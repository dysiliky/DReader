					...
					this.axios.get('http://txspring.cn:8010/addDetailGood?detailid=' + num)
						.then(function(response) {
							if(response.data != '1') {
								...
								console.log('请求成功');
								_this.$refs.upvote1.style.color = "#ffd900";
								_this.$refs.upvote2.style.color = "#ffd900";
				...
				this.axios.get('http://txspring.cn:8010/selectedUidAic?userid=' + getuserid + '&articalid=' + _this.article.detailpageid)
					.then(function(response) {
						if(response.data.type == '1') {
							...
							_this.$refs.storebt1.style.color = '#ffd900';
							_this.$refs.storebt2.style.color = '#ffd900';
							Toast({
								message: '该文章已经收藏过了！',
								position: 'center',
								duration: 1000
