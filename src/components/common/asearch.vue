<template>
  <div class="asearch">
    <mt-search placeholder="请输入关键字搜索" v-on:input="arefreshlist($event)" class="search">
    </mt-search>
  </div>
</template>
    ...
    methods: {
      ...
      arefreshlist(ev) {
        ...
        if (ipt.value.length > 0) {
          ...
          //   console.log(error)
          // })
          axios.get('http://txspring.cn:8010/getAllArticleDesc').then((res) => {
            let arr = []
            let data = res.data
            // console.log(data[0].detailpagetitle)
            for (let i = 0; i < data.length; i++) {
              if (data[i].detailpagetitle.indexOf(ipt.value) != -1) {
                arr.push(data[i])
              }
            }
            this.$emit('transferType', arr)
          }).catch((error) => {
            console.log(error)
          })
        }
